...
exports.run = function(client, message, args) {
  ...
  if (!message.guild) {
    return message.author.send('`temizle,sil` komutları sadece sunucularda kullanılabilir Veled Ben Salakmıyım.');
  }
  let mesajsayisi = parseInt(args.join(' '));
  if (mesajsayisi.length < 0) return message.channel.send('Kaç mesaj silmem gerektiğini belirtmedin Knk :smile:.')
  if (mesajsayisi > 100) return message.channel.send('100 adetden fazla mesaj silemem Aq Okadar da Deil!');
